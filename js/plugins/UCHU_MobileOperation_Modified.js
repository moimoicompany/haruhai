...
@desc シフト（ダッシュ）ボタンを表示する:true しない:false
初期値:true
@default true
@type boolean

@param ShiftBtn Size
@desc ダッシュボタンの大きさ(px）。 初期値:100
@default 100
@type number

@param ShiftBtn Margin
@desc ダッシュボタンの位置。画面端からの隙間の大きさで指定。
 (右からの幅; 下からの幅) 初期値:10; 0
@default 10; 0

@param ShiftBtn Orientation
@desc ダッシュボタンの基準位置を、右下以外に変えたい場合。
left か right; top か bottomで指定。 初期値:right; bottom
@default right; bottom
@type select
@option left; top
...
(function() {
	...
	PRM.url[3] = "./img/system/" + String(Parameters["ShiftBtn Image"])+ ".png";
	...
	PRM.visible[3] = Boolean(Parameters["ShiftBtn Visible"] === 'true' || false);
	PRM.size[3] = Number(Parameters["ShiftBtn Size"]);
	PRM.pos[3] = Parameters["ShiftBtn Margin"].split(";");
	PRM.spot[3] = Parameters["ShiftBtn Orientation"].split(";");
	...
	var btn_id=["DirPad","ok","escape","shift"];
	var current_zoom=1;	
	var st_x = 0;
	var st_y = 0;
	...
	Locate_DirPad.prototype.initialize = function() {
		...
		this.Div.addEventListener('touchstart', function(e) {
			...
			PressBtn=true;
			if (!SceneManager.isSceneChanging()){dirope(e.touches[num-1].clientX-dirx, e.touches[num-1].clientY-diry,true)}
		}, false);
		this.Div.addEventListener('touchmove', function(e) {
			var num = e.touches.length;
			if (num > 1){
				for (var i=0;i<num;++i){
					if(e.touches[i].clientX < 500){var num = i+1;}
				}
			}
			if (!SceneManager.isSceneChanging()){dirope(e.touches[num-1].clientX-dirx, e.touches[num-1].clientY-diry,false)}
		PressBtn=true;
		}, false);
	...
	//改変部分スイッチ168がONの時左右の移動禁止、スイッチ169がONの時上下の移動禁止
	function dirope(xx,yy,st) {
		...
		if(st && Math.sqrt(touchx*touchx+touchy*touchy)>1){
			...
			if(SceneManager._scene.constructor === Scene_Splash){disope();
			}else{
				if((touchx>Math.abs(touchy)*PRM.pad_dia)&&$gameSwitches.value(168) != true){Input._currentState['right']=true;Input._currentState['left']=false;}
				else if((touchx<-Math.abs(touchy)*PRM.pad_dia&&$gameSwitches.value(168) != true)){Input._currentState['left']=true;Input._currentState['right']=false;}
				else{Input._currentState['left']=false;Input._currentState['right']=false;}
				if((touchy>Math.abs(touchx)*PRM.pad_dia)&&$gameSwitches.value(169) != true){Input._currentState['down']=true;Input._currentState['up']=false;}
				else if((touchy<-Math.abs(touchx)*PRM.pad_dia)&&$gameSwitches.value(169) != true){Input._currentState['up']=true;Input._currentState['down']=false;}
	...
	Locate_Button.prototype.initialize = function(type) {
		...
		this.Div.addEventListener('touchend', function () {
			Input._currentState['ok'] = false;Input._currentState['escape'] = false;Input._currentState['shift']=false;PressBtn=false;
		}, false);
	...
	Scene_Base.prototype.start = function() {
	    ...
	    if (Utils.isMobileDevice() || PRM.pcBtn) {
			if(!Btn_ready){
				...
				if(PRM.visible[3]){this.shiftButton = new Locate_Button(3);}
        ...
        if(PRM.visible[0] || PRM.visible[1] || PRM.visible[2] || PRM.visible[3]){
            ...
            Graphics._updateRealScale = function() {
                ...
                if (this._stretchEnabled) {
                    ...
                    if(PRM.visible[3]){document.getElementById("shiftBtn").style.zoom=current_zoom;}
                    }
                }
            };
	}
	...
	// Option
        ...
        Scene_Base.prototype.hideUserInterface = function() {
            if (Utils.isMobileDevice() || PRM.pcBtn) {
                ...
                if(PRM.visible[3]){document.getElementById("shiftBtn").style.zIndex = '0';}
                if(PRM.hideBtnSwitch != 0){
                    ...
                    if(PRM.visible[3]){document.getElementById("shiftBtn").style.opacity = '0';}
                }
            }
        };
        ...
        Scene_Base.prototype.showUserInterface = function() {
            if (Utils.isMobileDevice() || PRM.pcBtn) {
                ...
                if(PRM.visible[3]){document.getElementById("shiftBtn").style.zIndex = '11';}
                if(PRM.hideBtnSwitch != 0){
                    //透明度を設定値にする処理
                    ...
                    if(PRM.visible[3]){document.getElementById("shiftBtn").style.opacity = PRM.opacity;}
                }
            }
        };
