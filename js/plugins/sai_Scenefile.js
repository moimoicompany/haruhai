...
　　sai.help_fontsize = Number(parameters['Help Font Size'] || 18);	
	...
	sai.members_x = Number(parameters['Members X'] || 80);	
	sai.members_y = Number(parameters['Members Y'] || 320);	
...
　  info.level = $gameParty.exists() ? $gameParty.members()[0].level : null;
	...
	info.saveString = $gameVariables.value(90);//2周目判定
...
//現状[ファイル名+ID、プレイ時間、地名、持ち金]、名称変更は''の中の文字で
    Window_SavefileStatus.prototype.drawFileId = function(id, x, y) {
      ...
      Window_SavefileStatus.prototype.drawContents = function(info, x, valid) {
		...
		if (info.location) {
		this.drawText(info.location, 100, Number(sai.location_y), 150);
		if(info.saveString == 0){
		}else{
			this.drawText(info.saveString, 0, 300, 150);//2周目表示用
		}
		}
		if (info.level) {
		...
		if (typeof info.gold === 'number') {　//値が数字の時に通る
			this.drawText(info.gold + " " + TextManager.currencyUnit, 100, Number(sai.gold_y), 200);
		}
