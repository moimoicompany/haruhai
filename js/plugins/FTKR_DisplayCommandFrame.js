...
(function() {
    ...
    FTKR.DCF.enabled = {
        ...
        reclist :Number(parameters['Enabled Recollection Command'] || 0),
        list : {
...
//この部分を変えました            var defIndex = item ? item.defIndex : 0;
	    var variable = $dataSystem.title1Name;
	    if (variable == 1){
	    var defIndex = 0;
	    }else if(variable == 4){
	    var defIndex = 1;
    ...
    */
    ...
    Window_TitleCommand.prototype.drawItem = function(index) {
        ...
        //CG観賞プラグインとの競合を避けるためにコマンド枠更新処理をタイトル画面の描写時に追加
               Window_Command.prototype.select = function(index) {
			        Window_Selectable.prototype.select.call(this, index);
			        this.updateDcfFrame(index);
				};
    ...
    var _DCF_Window_RecList_drawItem =  Window_RecList.prototype.drawItem;
     Window_RecList.prototype.drawItem = function(index) {
        if (FTKR.DCF.enabled.reclist) {
            this.changePaintOpacity(this.isCommandEnabled(index));
            this.drawDcfFrame(index);
        }
        _DCF_Window_RecList_drawItem.call(this, index);
    };
    //=============================================================================
    // Window_SkillList
    //=============================================================================
